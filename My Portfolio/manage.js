...
function getProjectId(item, index) {
  return item.id || ensureUniqueId(slugify(item.title, `project-${index + 1}`), draft && draft.projects ? draft.projects : []);
}

function getFrameId(item, index) {
  return item.id || ensureUniqueId(slugify(item.title, `frame-${index + 1}`), draft && draft.gallery ? draft.gallery : []);
}
...
async function savePendingPortfolioImages(nextData) {
  ...
  if (pendingPortraitImage) {
    nextData.portraitImage = await assetLibrary.saveImageWithFallback("portfolioAdmin", "portrait", pendingPortraitImage.file, `${slugify(nextData.name, "portfolio")}-portrait`, {
      maxWidth: 1800,
      maxHeight: 1800,
      quality: 0.84,
  ...
  if (pendingResumeDocument) {
    nextData.resumeUrl = await assetLibrary.saveAssetWithFallback("portfolioAdmin", "documents", pendingResumeDocument.file, `${nextData.name || "resume"}-resume`);
  }
  ...
  for (let index = 0; index < draft.projects.length; index += 1) {
    ...
    if (pendingImage) {
      nextData.projects[index].image = await assetLibrary.saveImageWithFallback(
        "portfolioAdmin",
        "projects",
        pendingImage.file,
        `${nextData.projects[index].id || `project-${index + 1}`}-cover`,
        {
          maxWidth: 1800,
          maxHeight: 1800,
          quality: 0.84,
    ...
    for (let docIndex = 0; docIndex < pendingDocs.length; docIndex += 1) {
      ...
      const savedUrl = await assetLibrary.saveAssetWithFallback(
        "portfolioAdmin",
        "documents",
        entry.file,
        `${nextData.projects[index].id || `project-${index + 1}`}-document-${docIndex + 1}`
      );
      documents.push({
  ...
  for (let index = 0; index < draft.gallery.length; index += 1) {
    ...
    for (let pendingIndex = 0; pendingIndex < pending.length; pendingIndex += 1) {
      ...
      const saved = await assetLibrary.saveImageWithFallback(
        "portfolioAdmin",
        "gallery",
        entry.file,
        `${nextData.gallery[index].id || `frame-${index + 1}`}-image-${pendingIndex + 1}`,
        {
          maxWidth: 1800,
          maxHeight: 1800,
          quality: 0.84,
...
function render() {
  ...
  renderSimpleList("project-list", "projects", (grid, item, index) => {
    ...
    grid.appendChild(createInput("LinkedIn Post Intro", item.postIntro || "", (value) => {
      draft.projects[index].postIntro = value;
    }, "textarea"));
    grid.appendChild(createInput("Key Highlights (one per line)", Array.isArray(item.highlights) ? item.highlights.join("\n") : "", (value) => {
      draft.projects[index].highlights = value.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
    }, "textarea"));
    grid.appendChild(createInput("Learning Outcome", item.outcome || "", (value) => {
      draft.projects[index].outcome = value;
    }, "textarea"));
    ...
    grid.appendChild(createInput("Tools & Focus Areas (comma separated)", Array.isArray(item.stack) ? item.stack.join(", ") : "", (value) => {
      draft.projects[index].stack = value.split(",").map((tag) => tag.trim()).filter(Boolean);
    }));
    grid.appendChild(createInput("Project Link", item.link, (value) => {
      draft.projects[index].link = value;
    }));
    grid.appendChild(createInput("LinkedIn Post Link", item.linkedinPostUrl || "#", (value) => {
      draft.projects[index].linkedinPostUrl = value;
    }));
    grid.appendChild(
      createImageEditor(
        "Project Image",
...
document.getElementById("add-project").addEventListener("click", () => {
  ...
  draft.projects.push({
    id: ensureUniqueId(`project-${count}`, draft.projects),
    title: `Project ${count}`,
    ...
    link: "#",
    linkedinPostUrl: "#",
    postIntro: "Sharing a recent project build.",
    highlights: ["Describe the first key highlight.", "Describe the second key highlight."],
    stack: ["Tool 1", "Tool 2"],
    outcome: "Add the main learning outcome or result from this project."
  });
  render();
  markDirty();
});
...
document.getElementById("add-gallery").addEventListener("click", () => {
  ...
  draft.gallery.push({
    id: ensureUniqueId(`frame-${count}`, draft.gallery),
