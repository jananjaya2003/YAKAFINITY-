...
function hasUsableLink(value) {
  return Boolean(value && value !== "#");
}

function projectCardMarkup(project, index) {
  const tags = Array.isArray(project.tags) ? project.tags : [];
  const detailHref = getProjectDetailHref(index);
  return `
    <a class="project-image-wrap" href="${detailHref}" aria-label="View ${project.title}">
    ...
    <div class="project-copy">
      ...
      <a class="project-link-button" href="${detailHref}">View Project</a>
    </div>
  `;
}
...
async function renderProjectDetail() {
  ...
  setText("project-post-title", project.title);
  setText("project-post-category", project.category || "Project");
  setText("project-post-intro", project.postIntro || `Sharing a recent project: ${project.title}.`);
  setText("project-post-built", project.fullDescription || project.summary);
  setText("project-post-outcome", project.outcome || "This project helped me improve how I plan, build, and present complete software work.");
  ...
  const highlightsRoot = document.getElementById("project-post-highlights");
  const highlightsSection = document.getElementById("project-post-highlights-section");
  if (highlightsRoot && highlightsSection) {
    const highlights = Array.isArray(project.highlights) ? project.highlights : [];
    highlightsSection.hidden = highlights.length === 0;
    highlightsRoot.innerHTML = highlights.map((item) => `<li>${item}</li>`).join("");
  }

  const stackRoot = document.getElementById("project-post-stack");
  const stackSection = document.getElementById("project-post-stack-section");
  if (stackRoot && stackSection) {
    const stack = Array.isArray(project.stack) && project.stack.length ? project.stack : Array.isArray(project.tags) ? project.tags : [];
    stackSection.hidden = stack.length === 0;
    stackRoot.innerHTML = stack.map((item) => `<span>${item}</span>`).join("");
  }
  ...
  const linkedinLink = document.getElementById("project-detail-linkedin-link");
  const linkedinCardLink = document.getElementById("project-detail-linkedin-card-link");
  const linkedinUrl = project.linkedinPostUrl || "#";
  if (linkedinLink) {
    if (hasUsableLink(linkedinUrl)) {
  ...
  if (linkedinCardLink) {
    if (hasUsableLink(linkedinUrl)) {
      linkedinCardLink.href = linkedinUrl;
      linkedinCardLink.textContent = "View LinkedIn post";
      linkedinCardLink.classList.remove("is-placeholder");
    } else {
      linkedinCardLink.href = "#";
      linkedinCardLink.textContent = "Add LinkedIn post link in portfolio manager";
      linkedinCardLink.classList.add("is-placeholder");
    }
  }
}
...
async function renderSite() {
  ...
  renderProjects("projects-grid", data.projects, 6);
