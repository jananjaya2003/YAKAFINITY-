...
const WHATSAPP_ORDER_NUMBER = "94721005844";
...
function buildServiceOrderMessage(serviceTitle, planName) {
  return buildWhatsAppLink([
    "Hello YAKAFINITY, I want to place an order.",
    serviceTitle ? `Selected service: ${serviceTitle}` : "",
    planName ? `Selected plan: ${planName}` : ""
  ]);
}
  ...
  forms.forEach((form) => {
    ...
    form.addEventListener("submit", (event) => {
      ...
      const whatsappLink = buildWhatsAppLink([
        ...
        `Selected service: ${order.serviceType || ""}`,
        plan ? `Selected plan: ${plan}` : "",
        `Budget: ${order.projectBudget || "Not set"}`,
        `Deadline: ${order.deadline || "Not set"}`,
        `Project details: ${order.details || ""}`
      ]);
      form.reset();
      if (message) {
        message.innerHTML = `Opening WhatsApp chat for your order. If it does not open, <a href="${whatsappLink}" target="_blank" rel="noopener">tap here</a>.`;
      }
...
function injectWhatsAppButton() {
  ...
  link.href = `https://wa.me/${WHATSAPP_ORDER_NUMBER}`;
  link.target = "_blank";
  link.rel = "noopener";
...
window.YakafinityOrderLinks = {
  buildServiceOrderMessage
};
