...
async function saveManagedSiteImage(folderName, file, nameBase) {
  return window.ProjectImageLibrary.saveImageFile("siteAdmin", folderName, file, nameBase);
}
...
function setupServiceForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const data = getSiteData();
    const previousData = structuredClone(data);
    const id = document.getElementById("serviceId").value || uid();
    ...
    if (file) {
      const imageBaseName = document.getElementById("serviceTitle").value.trim() || id;
      image = await saveManagedSiteImage("services", file, imageBaseName);
...
function setupOfferingForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const id = document.getElementById("offeringId").value || uidOffering();
    ...
    if (file) {
      image = await saveManagedSiteImage("items", file, name || id);
